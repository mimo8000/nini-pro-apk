...
export const DEFAULT_SUBSCRIBERS: SubscriptionUser[] = [
  ...
  },
  {
    code: 'NINI-ADMIN',
    tier: 'admin_unlimited',
    userName: 'ادمین اختصاصی ninipro',
    activatedAt: Date.now() - 15 * 86400000,
    expiresAt: null,
    trafficTotalGB: null,
    trafficUsedGB: 4.2,
    isAdmin: true,
    status: 'active',
    notes: 'دسترسی ادمین با قابلیت افزودن بی‌نهایت اشتراک',
  },
  {
    code: 'NINI-VIP',
    tier: 'vip_premium',
...
export function verifySubscriptionCode(code: string): { success: boolean; user?: SubscriptionUser; error?: string } {
  ...
  // Check admin matches and synonyms (English & Persian)
  const isAdminMatch =
    normalized === 'ADMIN' ||
    ...
    normalized === 'ROOT' ||
    normalized === 'NINI' ||
    normalized === 'NINI-ADMIN' ||
    normalized === 'NINIADMIN' ||
    normalized === 'ADMIN-UNLIMITED' ||
    normalized === 'ADMIN123' ||
    normalized === 'ADMINISTRATOR' ||
    cleanAlphaNumeric === 'ADMIN' ||
    cleanAlphaNumeric === 'NINIADMIN' ||
    raw.toLowerCase() === 'admin' ||
    raw.toLowerCase() === 'nini' ||
    raw === 'ادمین' ||
    raw === 'مدیر' ||
    raw === 'ادمین کل' ||
    raw.includes('ادمین');
  ...
  if (isAdminMatch) {
    const adminUser: SubscriptionUser = {
      code: 'ADMIN',
      tier: 'admin_unlimited',
      userName: 'مدیر کل (Master Admin)',
      activatedAt: Date.now(),
      expiresAt: null,
      trafficTotalGB: null,
      trafficUsedGB: 0,
      isAdmin: true,
      status: 'active',
      notes: 'دسترسی ویژه نامحدود ادمین',
    };
  ...
  if (!matched) {
    return {
      success: false,
      error: 'کد اشتراک وارد شده در سیستم ثبت نشده است. برای ورود ادمین می‌توانید کلمه ADMIN را وارد کنید.',
    };
  }
